...
public class bankDatabase {
    ...
    private HashMap<String, Customer> ReadFromCustomerDataBase(String filePath) {
        ...
        try (BufferedReader br = new BufferedReader(new FileReader(filePath))) {
            ...
            while ((line = br.readLine()) != null) {
                ...
                Customer newCustomer = new Customer(customerID, Password, customerName, customerNIC, address, phoneNo,
    ...
    private void updateCustomersToLocalDatabase(HashMap<String, Customer> customersDetail, String filePath) {
        ...
        try (BufferedWriter bw = new BufferedWriter(new FileWriter(filePath))) {
            bw.write(
                    "customerID,customerName,customerNIC,address,phoneNo,dateOfBirth,email,Password,accountNumbersOfCustomer\n");
            for (Customer customer : customersDetail.values()) {
                bw.write(customer.getCustomerID() + "," + customer.getCustomerName() + "," + customer.getCustomerNIC()
                        + "," + customer.getAddress() + "," + customer.getPhoneNo() + "," + customer.getDOB() + ","
                        + customer.getEmail() + ",");
    ...
    private void updateAccountsToLocalDatabase(HashMap<String, Account> accountsDetail, String filePath) {
        try (BufferedWriter bw = new BufferedWriter(new FileWriter(filePath))) {
            bw.write(
                    "acc_No,acctype,initialAmount,interestRate,customerName,customerID,branchName,branchID,PIN,currentStatus,createdDate,currentBalance\n");
            for (Account acc : accountsDetail.values()) {
                bw.write(acc.getAccNo() + "," + acc.getAccType() + "," + acc.getInitialAmount() + ","
                        + acc.getInterestRate() + "," + acc.getCustomerName() + "," + acc.getCustomerID() + ","
                        + acc.getBranchName() + "," + acc.getBranchID() + "," + acc.getPIN() + "," + acc.getAccStatus()
                        + "," + acc.getCreatedDate() + "," + acc.getBalanceForDB() + "\n");
            }
        } catch (IOException e) {
            System.out.println("Error writing to file: " + e.getMessage());
        }
    ...
    public void addNewBrancheToLocalDatabase(Branch branchesDetail, String filePath) {
        ...
        try (BufferedWriter bw = new BufferedWriter(new FileWriter(filePath, true))) {
            bw.write(branchesDetail.getBranchID() + "," + branchesDetail.getBranchName() + ","
                    + branchesDetail.getContactNo() + "," + branchesDetail.getLocation() + ","
                    + branchesDetail.getBranchEmail() + "," + branchesDetail.getBranchManagerName() + ","
                    + branchesDetail.getBranchManagerEmail() + "," + branchesDetail.getBranchManagerPhoneNo() + "\n");
        } catch (IOException e) {
            System.out.println("Error writing to file: " + e.getMessage());
        }
    }
    ...
    public void localDBUpdate(HashMap<String, Customer> customerDatabase, String custoFilePath,
            ...
            HashMap<String, Transaction> transactionDatabase, String transFilePath) {
    ...
    public void DBInitialize(HashMap<String, Customer> customerDatabase, String custoFilePath,
        ...
        transactionDatabase = this.ReadFromTransactionDataBase(transFilePath);
    }
    ...
    // for (Transaction transaction : db.transactionDatabase.values()) {
